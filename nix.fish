...
function __fish_nix_completions_print_current_top
    function __fish_nix_completions_pda_step --argument token --no-scope-shadowing
        ...
        for rule in $rule_list
            set rule (string replace --all ";" \n -- $rule) # split on newlines to obtain array
...
function __fish_nix_completions_create_completions --argument cmd
    ...
    set rule_list (string replace --all "; " ";" -- $rule_list) # Normalize
    set rule_list (string replace --all " ;" ";" -- $rule_list) # Normalize
    set quoted_rules
    for rule in $rule_list
        ...
        set rule (string replace --all ";" \n -- $rule) # split on newlines to obtain array
        set -e rule[-1]
        set quoted_rules $quoted_rules "'"(string replace --all " " ";" -- (echo $rule))"'"
    end
    for rule in $rule_list
        test -n "$rule"; or continue
        set rule (string replace --all ";" \n -- $rule) # split on newlines to obtain array
    ...
    complete -f -c $cmd -n "__fish_nix_completions_current_top_is_not_fd $quoted_rules"
end
